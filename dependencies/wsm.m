WWWWWW = whos; lkmemory(WWWWWW); clear WWWWWW

% lkmemory displays the number of bytes that the current variables in the
% workspace take up (i.e. a sum of the third column of "whos")
function lkmemory(varargin)
if isempty(varargin)
    disp('Execute this:    W = whos; lkmemory(W)')
elseif length(varargin) == 1
    if isstruct(varargin{1})
        ...
        error('The input for lkmemory must be a "struct" generated by running "whos".')
    end
else
    error('lkmemory only accepts 0 or 1 argument')
end
end
