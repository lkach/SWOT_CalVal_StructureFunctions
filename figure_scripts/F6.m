...
warning('Be sure that the GPS data is packaged with the other data (or ideally SIO and PMEL will send us an updated version).')
...
% % % Uncomment this if you want to visualize the depths to which the
% % % profilers go in order to determine a reasonable cutoff depth.
...
% The depths we use as reasonable cutoffs:
SP_RANGE_LIM = 477;
PP_RANGE_LIM = 414;

%% Visualize the depths of the fixed CTDs

% % % Uncomment this if you want to visualize the depths at which the
% % % fixed CTDs are located.
...
%% SPIKES
% Obviously the spike in S2 is wrong and was somehow not filtered out.
...
% % % Uncomment to visualize the segments of data and their windowed
% % % versions before taking their FFTs to estimate spectral power:
...
loglog(FF(1:dsearchn(FF,1)), ...
       [10^-2.5]*[FF(1:dsearchn(FF,1))].^-3, 'k--','LineWidth',LineWidth*3,'HandleVisibility','off')
text(10^-1,10^-1.6,     '$\propto\omega^{-3}$','FontSize',20,'Interpreter','LaTeX')
...
%%
% exportgraphics(gcf,...
%     '.../CalVal_StructureFunctions/figures/F5.pdf',...
%     'BackgroundColor','none','ContentType','vector')
% disp(['Image saved'])

% % % Revised figure:
exportgraphics(gcf,...
    '/Users/kachelein/Documents/JPL/papers/my_work/CalVal_StructureFunctions/REV1/F6.pdf',...
    'BackgroundColor','none','ContentType','vector')
disp(['Image saved'])
...
% % % Uncomment if this analysis is wanted:
