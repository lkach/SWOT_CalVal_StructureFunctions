%% Supplemental Figure
% Look at the relative small California SWOT Cal/Val diamond for SWOT data:

cd /Users/kachelein/Documents/JPL/papers/my_work/CalVal_StructureFunctions/data/SWOT_Other_Crossovers

% Small rectangles
% DIR = dir('./SWOTdata_small*.mat');

% Large rectangles
DIR = dir('./SWOTdata_large*.mat');

for xo_ii = 1:length(DIR) % 1:length(DIR)

SWOT = load([DIR(xo_ii).folder '/' DIR(xo_ii).name],...
...
% end
% 
% SPEC_MEAN_track = SPEC_MEAN_track(:,SPEC_MEAN_track(1,:)>1);
% 
% SPEC_MEAN   = mean(  SPEC_MEAN_track,2,'omitnan');
% SPEC_MEDIAN = median(SPEC_MEAN_track,2,'omitnan');
% 
% % figure
...
loglog( Freq(1:dsearchn(Freq,0.02)), ...
        10^-7.3 * Freq(1:dsearchn(Freq,0.02)).^[-3], 'k--', 'LineWidth',3,'HandleVisibility','off')
text(Freq(1)*12,10^-1.9,'\propto k^{-3}','FontSize',16,'HandleVisibility','off')
...
exportgraphics(gcf,...
['/Users/kachelein/Documents/JPL/papers/my_work/CalVal_StructureFunctions/figures/Other_Crossovers/' ...
 DIR(xo_ii).name(1:[end-4]) '_PowerSpectrum_MeanRemoved.pdf'],...
...
for jj = 1:180 % 1:length(SWOT.ssha_karin_2)
...
title({ DIR(xo_ii).name ; ...
    ['Spectral slope inferred from SF up to ' num2str(DIST_VEC(20)) ' km = ' num2str(-1 - NLLSF_COEF(2))]})

%% Save

exportgraphics(gcf,...
['/Users/kachelein/Documents/JPL/papers/my_work/CalVal_StructureFunctions/figures/Other_Crossovers/' ...
 DIR(xo_ii).name(1:[end-4]) '_StructureFunction.pdf'],...
'BackgroundColor','none','ContentType','vector')
...
LEG_HANDLE.Position = [0.6368    0.8559    0.1875    0.1378];

%% Export the figure

exportgraphics(gcf,...
['/Users/kachelein/Documents/JPL/papers/my_work/CalVal_StructureFunctions/figures/Other_Crossovers/' ...
 DIR(xo_ii).name(1:[end-4]) '_SlopeVersusCrosstrack.pdf'],...
'BackgroundColor','none','ContentType','vector')

close all

disp(['Done with ' DIR(xo_ii).name])
pause(1)
